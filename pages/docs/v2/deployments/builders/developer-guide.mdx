...
export const meta = {
  ...
  lastEdited: '2019-03-22T22:53:49.000Z'
}
...
- The `build` function returns a [Files](#files) datastructure that contains the [build outputs](/docs/v2/deployments/builds#sources-and-outputs), which can be:
  - Static files
...
- `files`: All source files of the project as a [Files](#files) datastructure
...
All the APIs you export (`analyze`, `build` and `prepareCache`) are not guaranteed to be run in the same process, but the filesystem we expose (e.g.: `workPath` and the results of calling [`getWriteableDirectory`](#getWriteableDirectory) ) is retained.
...
// Deprecated usage: `@now/build-utils/file-ref`
```
...
// Deprecated usage: `@now/build-utils/file-fs-ref`
```
...
// Deprecated usage: `@now/build-utils/file-blob`
```
...
// Deprecated usage: `@now/build-utils/lambda`
```
...
// Deprecated usage: `@now/build-utils/fs/download`
```
...
// Deprecated usage: `@now/build-utils/fs/glob`
```

This utility allows you to _scan_ the filesystem and return a `Files` representation of the matched glob search string. It can be thought of as the reverse of [`download`](#download).
...
// Deprecated usage: `@now/build-utils/fs/get-writable-directory`
```
...
// Deprecated usage: `@now/build-utils/fs/rename`
```
